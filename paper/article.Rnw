...
f^*)(\{a\}))$, where $\circ$ denotes function composition.  Since $f^*$ always
receives a singleton set, its definition as a function of the powerset of the
group labels $A$ is too general for this algorithm.  However, this
...
\begin{equation}
U\xonto{f_0}A_0\xonto{f_1}A_1\xonto{f_2}\cdots\xonto{f_n}A_n.
...
Denote with $F_k:A_0\to A_k$, the function that  accepts a label in $A_0$ and
...
function accepts a set of labels in $A_k$ and returns all the labels in $A_0$
...
\Input{A finite set $U$, an aggregator $\phi: 2^U\to X$, a test function $\beta: 2^U\to \mathbb{B}$,
      and a collapsing sequence $U\xonto{f_0}A_0\xonto{f_1}\cdots \xonto{f_n}  A_n$}
...
\For{$a\in A_0$}{
  ...
  $d = f_0^*(\{a\})$ \tcp*{Get subset of $U$}
  \While{$i<n \land \lnot\beta(d)$}{ \label{line:while}
    $i = i+1$ \tcp*{Increase collapse level} 
    $d = (f_0^*\circ F_i^*\circ F_i)(a)$ \tcp*{Collapse and get subset}
  }\label{line:endwhile}
...
a value for each member of $A_0$ will be found. For each member of $A_0$ where
an aggregate is computed, there is a triple $(a,i,\phi(d))$, where $a\in A_0$
is the label to which the value $\phi(d)$ pertains, and $i$ is the number of
collapses applied to reach a suitable dataset. 

\subsection{Performance, memory, parallelization}
...
Generic parallelization of Algorithm~\ref{alg:sac} can be done by parallelizing
...
that a generic parallelization scheme would need to ensure that each worker
node has access to the full dataset $|U|$. There are two types of options to do
this: either multliple workers gain read-only access to $U$ or each worker gets
...
generic parallelization needs to either consider a specific use case (e.g.
