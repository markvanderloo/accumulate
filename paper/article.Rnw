...
Generalizing further, one may be interested in estimating (statistical) models.
With the \code{cumulate} function it is possible to specify complex aggregates
that go beyond a simple number. Below, we estimate the following linear model
\begin{displaymath}
