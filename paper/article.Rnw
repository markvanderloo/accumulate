...
groups. We say that $f:U\to A$ is a \emph{partition} of $U$.
...
$A\to X$ since it pairs each $a\in A$ with one element of $X$). We also
introduced some new notation: the notation $2^U$ is the power set of $U$ and
$f^{-1}$ stands for the formal \emph{inverse} of $f$. That is: you give it a
set of labels from $A$ and it returns all the elements of $U$ that have any of
those labels. This definition is somewhat too general for the current
algorithm, but it will be necessary when we move to collapsing groups.

Let us see how the elements $U$, $f:U\to A$, and $\phi$ are implemented in
practice. Consider the signature of the R \code{aggregate()} function for data
