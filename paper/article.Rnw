...
\proglang{Rust} also implements such features. Similarly, the \pkg{DataFrames}
...
cumulate(input1, collapse=csh, test=min_records(3), muY=mean(Y), muY2 = mean(Y2))
@
...
accumulate(input, collapse = A*B ~ A*B1 + B1, test=my_test, fun=mean)
@
...
f^*(S) = \{u\in U|f(u)\in S\},
\end{equation*}
where $S$ is a subset of $A$ (See e.g. \citet[Section 1.4]{fong2019invitation}).
