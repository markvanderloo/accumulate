...
meaning that one estimates parameters for a larger subpopulation which are then
used in the estimate for the target subpopulation.  Here, we shall be
interested in estimating the average turnover from industrial activities
(\code{industrial}) by SBI and size class.
