...
is necessary. There are two main functions in \pkg{accumulate} that offer
slightly different interfaces.
\begin{verbatim}
  accumulate(data, collapse, test, fun, ...)
  cumulate(data, collapse, test, ...)
\end{verbatim}
Here \texttt{data} is a data frame, \texttt{collapse} represents the collapse
sequence (as a \class{formula} or a \class{data frame}), and \texttt{test} is a
function that accepts a subset of \texttt{data} and returns a boolean that
indicates whether a subset is suited for aggregation. In \code{accumulate()},
the parameter \code{fun} represents an aggregation function that is applied to
every column of \code{data}, and the ellepsis (\code{...}) is for arguments
that are passed as extra argument to \code{fun}. The interface of
\code{accumulate()} is modeled after the \code{aggregate()} function in
\proglang{R}.  In \code{cumulate}, the ellepsis is a sequence of
comma-separated \code{name=expression} pairs in the style of \code{summarise()}
...
schemes. For example when grouping by two categorical variables $A$ and
$B$, one may first want to collapse variable $B$ into a
variable $B1$ and if that is not sufficient, grouping
in the second variable is dropped so grouping is only defined by $A$.
In \code{accumulate()} such a scheme is denoted with the formula \code{A*B \~{}
...
levels separated by \code{+}.

The following tables illustrate the idea. The left table represents the data
set to be aggregated by $A$ and $B$. In this example we wish to
compute the sum $t_Y$ of column $Y$ per group. We demand that there are at
least three records in a group, otherwise we collapse one step (we set values
of $Y$ to unique powers of two, so it is clear which records are used
to compute each value of $t_Y$).
%
\begin{center}
\begin{tabular}{cccl}
...
$(A=1,B=11)$. The collapsing level is zero, which means
that no collapsing was necessary. Indeed, in the data table we see that there
are three rows with $A=1$ and $B==11$ with $Y$ values $1, 2$,
and $7$, resulting in $t_Y=7$ for this group.

Next, the algorithm tries to compute the total for group
$(A=2,B=12)$ (in green) but finds that there are only two
such rows. It now defines a new group $(A=2,B1=1)$ and finds
that there are three records in that group so it computes $t_Y=8+16+32=56$.
Similarly, it finds one record with $(A=2,B=13)$ and
collapses to $(A=2,B1=1)$, again yielding $t_y=56$.

Finally, for $(A=2,B=21)$ the algorithm finds only a single
record. It collapses to $(A=2,B1=2)$ and finds two so it
collapses further to $(A=2)$ to find three records.  This yields
$t_Y=64+128+256=448$. Similarly, the groups $(A=3, B=22)$ and
$(A=3,B=12)$ are collapsed to $(A=2)$.
...
It is possible to get the same result with \code{accumulate()}. This will 
cause summation over all variables that are not used in the formula object. In
the below example we also introduce the helper function \code{min_records()}.
<<>>=
input$Y2 <- 3^(0:8)
...
In order to use the data frame interface, the input dataset needs to have the
most fine-grained grouping variable. In the running example this is
$A\times B$, so we need to combine that into a single variable, and remove
the other ones.
<<>>=
input$AB <- paste(input$A, input$B, sep="-")
input <- input[-(1:3)]
input
@
...
<<>>=
accumulate(input, collapse=csh, test = min_records(3), sum)
cumulate(input, collapse=csh, test=min_records(3), tY=sum(Y), tY2 = sum(Y2))
@
...
\section{Extensive example: imputing economic data} \label{sect:example}
