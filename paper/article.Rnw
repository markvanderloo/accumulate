...
subset collapsing scheme. Finally, we demonstrate and analyze the pseudocode
...
objects. Similarly, the \pkg{DataFrames} package for \proglang{julia}
implements such functionality \citep{kaminski2022dataframes}.
...
subset, where the number of (sampled) observations so small that the variance
...
We describe the fundamental algorithm in pseudocode and compare it with the
...
imputation value is copied from a (possibly random) donor record with
attributes that are sufficiently similar \citep{andridge2010review}.
One way to generate an imputation value is to select a random donor
from the same group. To prevent the same donor from being used too
often, it is not uncommon to demand a minimum number of records
in the donor pool. A collapsing scheme is one way of guaranteeing
this, and below we demonstrate how this problem can be expressed
in \code{accumulate}.
...
@
To make sure we obtain a donor for each record, we add an
identifying column to use as grouping variable.
\code{id} column.
<<>>=
...
Denote with $U$ be a finite set, and let $\phi:2^U\to X$ be a function that
...
always receives a singleton set, its definition as a pullback along $f$ more
general then necessary for this algorithm.  However, this formulation turns out
to be necessary when generalizing the algorithm to the case of dynamic
grouping.
...
It is not hard to see that in this case Algorithm~\ref{alg:saccg} reduces to
Algorithm~\ref{alg:sac}. The worst case is achieved by setting $\beta: d\mapsto
...
application of the pullback $f^*$. First note that there is no difference in
applying the $f^*$ or $f^*\circ F_i^*\circ F_i$: in practice a nested function
call is not necessary as the collapsing scheme is fully available as input. The
pullback performs a lookup of records based on matching one or more attributes
with a set of attribute values. The time complexity of such operations are
typically reduced by proper preparation of the dataset. For example, in
databases one may define indices that speed up certain often-used lookup
operations (that then may rely on those indices). In the implementation of the
\code{accumulate()} package the pullback is (currently) implemented using
standard join operations as implemented by \proglang{R}'s standard
\code{merge()} function.  This implies that the time complexity of
\code{accumulate()} and \code{cumulate()} should be multiplied with the time
dependence of \code{merge()}.
...
dynamically determined based on a user-defined conditions and a user-defined
collapsing scheme. We demonstrated that these interfaces can be used in
situations including multiple grouping variables and complex collapsing
schemes, as well as situations where multiple variables need to be aggregated
over. Moreover, the package supports the collection of complex aggregates, such
as model outputs and offers several service functions that aim to facilitate
the definition of the collapsing conditions.

Finally, the pseudocode underlying the package's main functions has been
formally analyzed and it is shown that aggregation with collapsing groups can
be interpreted as a generalization of standard grouped aggregation in a very
precise sense. We hope that this stimulates broader implementation of this
algorithm in software offering grouped aggregation.
